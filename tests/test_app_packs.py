...
def _install_lua(store, tmp_path, name, body, world):
    src = tmp_path / name
    src.write_text(body, encoding="utf-8")
    store.install(src, world=world)
...
def test_cli_install_list_conflicts_roundtrip(tmp_path, capsys):
    ...
    assert packs_main(["--root", root, "install", str(pack), "--world", "mud"]) == 0
...
def test_cli_conflicts_reports_key_clash_with_nonzero_exit(tmp_path, capsys):
    ...
    for name in ("a.lua", "b.lua"):
        ...
        packs_main(["--root", root, "install", str(path), "--world", "mud"])
    capsys.readouterr()
